...
func listConnections(
	...
	vydonurl := auth.GetVydonUrl()
	httpclient, err := auth.GetVydonHttpClient(ctx, logger, auth.WithApiKey(apiKey))
	if err != nil {
		return err
	}

	userclient := mgmtv1alpha1connect.NewUserAccountServiceClient(httpclient, vydonurl)
	...
	connectionclient := mgmtv1alpha1connect.NewConnectionServiceClient(httpclient, vydonurl)
