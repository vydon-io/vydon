...
func NewCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if apiKey != "" {
				logger.Info(
					`found api key, no need to log in. run "vydon whoami" to verify that the api key is valid`,
				)
				return nil
			}
...
func login(ctx context.Context, logger *slog.Logger) error {
	...
	authclient := mgmtv1alpha1connect.NewAuthServiceClient(httpclient, auth.GetVydonUrl())
	...
	httpclient, err = auth.GetVydonHttpClient(ctx, logger)
	if err != nil {
		return err
	}
...
func oAuthLogin(
	...
	if err := webbrowser.Open(authorizeurlResp.Msg.Url); err != nil {
		fmt.Println( //nolint:forbidigo
			"There was an issue opening the web browser, proceed to the following url to finish logging in to Vydon:\n",
