...
func Get(
	...
	generatorFns, err := getVydonGenerators()
	if err != nil {
		return nil, err
	}
	transformerFns, err := getVydonTransformers(transformPiiTextApi)
	if err != nil {
		return nil, err
	}
			...
			return func(ctx context.Context, call goja.FunctionCall, rt *goja.Runtime, l *slog.Logger) (result any, err error) {
				defer func() {
					if r := recover(); r != nil {
						...
						l.Error(
							"recovered from panic in custom vydon function",
							"error", err,
...
func getVydonGenerators() ([]*javascript_functions.FunctionDefinition, error) {
	generators := transformers.GetVydonGenerators()
...
func getVydonTransformers(
	transformPiiTextApi transformers.TransformPiiTextApi,
) ([]*javascript_functions.FunctionDefinition, error) {
	vydonTransformers := transformers.GetVydonTransformers()
	if transformPiiTextApi != nil {
		vydonTransformers = append(vydonTransformers, transformers.NewTransformPiiText(transformPiiTextApi))
	}
	fns := make([]*javascript_functions.FunctionDefinition, 0, len(vydonTransformers))
	for _, f := range vydonTransformers {
		templateData, err := f.GetJsTemplateData()
		if err != nil {
			return nil, err
		}
