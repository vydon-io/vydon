...
func Test_VydonArray(t *testing.T) {
	t.Run("NewVydonArray initializes with correct defaults", func(t *testing.T) {
		elements := []VydonAdapter{
			&Interval{},
			&Interval{},
		}
		array, err := NewVydonArray(elements)
		require.NoError(t, err)
		require.NotNil(t, array)
		require.Equal(t, VydonArrayId, array.Vydon.TypeId)
		require.Equal(t, LatestVersion, array.GetVersion())
		require.Equal(t, len(elements), len(array.Elements))
	})

	t.Run("NewVydonArray accepts options", func(t *testing.T) {
		elements := []VydonAdapter{&Interval{}}
		customVersion := Version(1)
		array, err := NewVydonArray(elements, WithVersion(customVersion))
		require.NoError(t, err)
		require.Equal(t, customVersion, array.GetVersion())
	})

	t.Run("NewVydonArray with invalid version returns error", func(t *testing.T) {
		elements := []VydonAdapter{&Interval{}}
		invalidVersion := Version(999)
		array, err := NewVydonArray(elements, WithVersion(invalidVersion))
		require.Error(t, err)
		require.Nil(t, array)
		require.Contains(t, err.Error(), "invalid Vydon Type version")
	})
	...
	t.Run("ScanPgx handles valid intervals", func(t *testing.T) {
		interval1 := &Interval{}
		interval2 := &Interval{}
		array, err := NewVydonArray([]VydonAdapter{interval1, interval2})
		require.NoError(t, err)
