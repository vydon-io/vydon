...
func Test_TypeRegistry(t *testing.T) {
	t.Run("NewTypeRegistry initializes with default types", func(t *testing.T) {
		...
		interval, err := registry.New(VydonIntervalId, LatestVersion)
		require.NoError(t, err)
		require.NotNil(t, interval)

		array, err := registry.New(VydonArrayId, LatestVersion)
		require.NoError(t, err)
	...
	t.Run("Register adds new type successfully", func(t *testing.T) {
		...
		registry.Register(customTypeId, customVersion, func() (VydonAdapter, error) {
	...
	t.Run("New falls back to latest version when specific version not found", func(t *testing.T) {
		...
		obj, err := registry.New(VydonIntervalId, nonExistentVersion)
		require.NoError(t, err)
		require.NotNil(t, obj)
	})
	...
	t.Run("Unmarshal handles regular JSON without _vydon field", func(t *testing.T) {
		registry := NewTypeRegistry(testutil.GetTestLogger(t))
	...
	t.Run("Unmarshal handles VydonInterval type", func(t *testing.T) {
		registry := NewTypeRegistry(testutil.GetTestLogger(t))
		intervalJSON := []byte(`{
			"_vydon": {
		...
		interval, ok := result.(VydonAdapter)
		require.True(t, ok)
		require.NotNil(t, interval)
	})

	t.Run("Unmarshal handles VydonInterval type in map[string]any", func(t *testing.T) {
		registry := NewTypeRegistry(testutil.GetTestLogger(t))
		intervalMap := map[string]any{
			"_vydon": map[string]any{
				"type_id": "VYDON_INTERVAL",
				"version": 1,
			},
			"microseconds": 1,
	...
	t.Run("Unmarshal handles VydonArray type", func(t *testing.T) {
		registry := NewTypeRegistry(testutil.GetTestLogger(t))
		arrayJSON := []byte(`{
			"_vydon": {
				"type_id": "VYDON_ARRAY",
	...
	t.Run("Unmarshal returns error for non-VydonAdapter array elements", func(t *testing.T) {
		registry := NewTypeRegistry(testutil.GetTestLogger(t))
		...
		require.Error(t, err)
		require.Nil(t, result)
		require.Contains(t, err.Error(), "not a VydonAdapter")
	})
}
