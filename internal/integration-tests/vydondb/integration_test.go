...
func (s *IntegrationTestSuite) SetupSuite() {
	...
	s.db = vydondb.New(s.pgcontainer.DB, db_queries.New())
}
...
func (s *IntegrationTestSuite) TearDownTest() {
	...
	_, err := s.pgcontainer.DB.Exec(s.ctx, "DROP SCHEMA IF EXISTS vydon_api CASCADE")
	if err != nil {
		panic(err)
	}
...
func (s *IntegrationTestSuite) Test_SetUserByAuth0Id() {
	...
	t.Run("idempotent", func(t *testing.T) {
		...
		uid1 := vydondb.UUIDString(resp.ID)
		uid2 := vydondb.UUIDString(resp2.ID)
...
func (s *IntegrationTestSuite) Test_SetPersonalAccount() {
	...
	t.Run("idempotent - parallel", func(t *testing.T) {
		...
		errgrp.Go(func() error {
			resp, err := s.db.SetPersonalAccount(errctx, user.ID, &maxAllowed)
			assertNoErrResp(t, resp, err)
			uid2 = vydondb.UUIDString(resp.ID)
			return nil
		})
...
func (s *IntegrationTestSuite) Test_ConvertPersonalToTeamAccount() {
	...
	t.Run("success", func(t *testing.T) {
		...
		newTeamName := "newteam"
		resp, err := s.db.ConvertPersonalToTeamAccount(s.ctx, &vydondb.ConvertPersonalToTeamAccountRequest{
			UserId:            user.ID,
...
func (s *IntegrationTestSuite) Test_CreateAccountApiKey() {
	...
	key, err := s.db.CreateAccountApikey(s.ctx, &vydondb.CreateAccountApiKeyRequest{
		KeyName:           "foo",
		KeyValue:          "bar",
...
func (s *IntegrationTestSuite) Test_CreateJob() {
	...
	}, []*vydondb.CreateJobConnectionDestination{
		{
