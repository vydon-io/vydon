...
MongoDB is one of the most commonly used NoSQL database in the world and Vydon natively supports it.
...
Vydon's compose file has an option to stand up a local MongoDB instance. If you're using the `compose.dev.yml` file to stand up Vydon locally, uncomment the MongoDb line in order to stand up a MongoDB instance:
...
Your setup will vary based on your specific settings, but generally, you can configure Vydon to connect to Mongo by providing a Client Certificate and Client Key in their respective fields in the Connection(s) form.

By doing so, Vydon will store them on disk and update the connection url to point to those files so that they can be used to connect.
...
When configuring your MongoDB user, please ensure that you grant it read permissions to relevant Database Collections that you want Vydon to have access to.

### Destination Connections

When configuring your MongoDB user, please ensure that you grant it necessary write permission to the relevant Database and Collections that Vydon will need access to.
