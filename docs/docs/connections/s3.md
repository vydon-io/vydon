---
title: S3
description: Learn how to use Vydon to sync data from a database to AWS S3
id: s3
hide_title: false
slug: /connections/s3
---
...
> Screenshot pending re-hosting. Open the Vydon UI and create an AWS S3 connection to see this form.
...
When data is synced to your S3 bucket using Vydon, the output will be stored in JSON arrays compressed into gz files. The files are organized in the following path structure:
...
This format allows Vydon CLI to easily stream records from the file line by line as opposed to downloading the entire json blob, decoding it, and then beginning record insertion.
