...
description: Learn how to add virtual foreign keys in Vydon
...
Vydon supports virtual foreign keys, which can be added using the Vydon app. Virtual foreign keys allow you to create relationships between tables that do not have explicit foreign key constraints in the database. This is particularly useful when dealing with legacy databases or when you need to create temporary relationships for data syncing purposes.

To add a virtual foreign key in Vydon, follow these steps:

**Navigate to the Schema Mapping Page:**

- Go to the Transformer Mapping page in the Vydon app where you define your data mappings. Click on the `Virtual Foreign Keys` tab.
...
- After defining the virtual foreign keys click update to save. The Vydon app will now treat these columns as if they had a real foreign key constraint, maintaining the integrity of your data during syncing operations.
