...
description: Learn how to use Vydon to initialize your schema in your destination database to make it easier to hydrate your database with data
...
Vydon has the ability to initialize your database schema prior to running a sync. This is useful if you have a lot of migrations or if you want to ensure that your schema is exactly the same as your source.
...
Once this is enabled, Vydon will generate SQL statements based on the schemas and tables you've selected in your job's configuration to be created prior to syncing the data itself.

Vydon tries to be as smart as possible about what to include in the schema, but it is not always perfect. If you find that you are missing something, you can always manually add it to the schema, or reach out to us to help you out.
...
- Have hard coded database names (if you are choosing to run Vydon against a separate logical database in your single instance)
...
Steps to run this method are out of scope for the Vydon docs, but ideally it is as simple as running your migrations against the same database as the Vydon destination.
...
- You can remove certain things that cause hangups with Vydon (and inserts in general) like certain triggers, views, and permissions.
...
Then edit the `my_schema.sql` file to remove anything that is tripping up Vydon, save it in a separate file, and execute it against your target database after the sync is finished.
