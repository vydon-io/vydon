---
title: Troubleshooting Vydon Jobs
description: Learn how to troubleshoot common errors in Vydon with this guide
id: troubleshooting
hide_title: false
...
Databases are _complex_. Vydon does its best to handle any and all edge cases you throw at it. This document is a collection of common issues you may face when running Vydon and how to resolve them.

It's worth reviewing the Vydon [Platform](/platform) overview to understand Vydon's architecture and where possible issues can come up. Generally speaking, issues can come from three different places:
...
You can resolve this by reducing the number of open connections in the "Connections" tab of Vydon. The default is at 50, the easiest way to debug is to start with something small like 10 and increase it until you hit the error again.
...
An activity timeout defines the maximum time allowed for a single activity. In Vydon, an activity is responsible for syncing one table. If that table is very big and takes longer than the activity timeout, then you may see this error.
...
Vydon does not support creating schemas on the fly. You must create the schema on the target database before running the job.

### `unable to exec truncate cascade statements: ERROR: relation \"public.table_name\" does not exist`

When you have "Truncate" enabled in the Source Options, Vydon will attempt to truncate the table before inserting new records. If the table does not exist, this error will be thrown.

You can resolve this by disabling the "Truncate" option in the Source Options, and enabling the "Init Table Schema" for your first run, and Vydon will attempt to create the table for you.
...
Some Linux environments have issues with Docker Networking, which may cause problems with Vydon communications.
...
Vydon currently supports debugging database queries for Postgres connections.

Database query logging is by default turned off, which is equivalent to setting `DB_LOG_LEVEL=none`.

This log level works similar to Vydon's standard `LOG_LEVEL` where you can set values to emit certain levels of database logging.
