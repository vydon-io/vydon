---
title: Account Hooks
description: Learn how to use Account Hooks to add further customization to your Vydon account
id: account-hooks
hide_title: false
...
When an event is emitted, Vydon retrieves all active hooks for the event and executes each hook in order of creation. Today they are executed in synchronous fashion, meaning that each hook is executed one at a time. This may change in the future to allow for parallel execution of hooks.
...
The secret key is set when the webhook is created and is used to verify that the webhook is coming from Vydon.

The HMAC hash is generated using the SHA256 algorithm and the secret key.

The HMAC hash is sent in the `X-Vydon-Signature` header.

The HMAC hash algorithm is sent in the `X-Vydon-Signature-Type` header.
...
const WEBHOOK_SIG_HEADER = "X-Vydon-Signature"
...
Today the messages are fully controlled by Vydon and cannot be altered.

Configuring the Slack hook requires using Oauth to install the `Vydon` Slack App into your Slack's workspace.
...
> For private slack channels, the Vydon app will have to be manually invited to it, as Slack does not provide a way to automatically do this.
