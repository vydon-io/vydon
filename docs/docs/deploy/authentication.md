...
description: Learn how to configure and maintain authentication within Vydon open source
id: auth
hide_title: false
...
By default, Vydon launches without requiring any form of authentication. This is to make getting started with Vydon easier, and it's not advised to run Vydon in production without any form of authentication.
...
Internally, we utilize Auth0, and to date this is the only provider that has been fully tested for use with Vydon.
...
If there is any trouble configuring authentication with Vydon, reach out to us for help on Discord.

## How to Authenticate against Vydon API
...
This form of authentication also makes writing scripts with the Vydon SDKs much easier and it isn't as easy to provide a user access token.

API Keys have their own Vydon User Identifier assigned to them and are scoped to the Vydon Account.
...
1. navigate to the relevant instance of Vydon
...
Vydon API and Vydon Worker both require mTLS authentication when interfacing with Temporal (if this is enabled in Temporal).
Like Vydon, by default, the local versions of Temporal don't require authentication by default (although this may be changing.)
...
Vydon currently supports keycloak and auth0 for this feature.
...
- `AUTH_API_BASEURL` - This is the base url for the Admin API. Auth0 calls this the Management API, while Keycloak the Admin API.
  ...
  - For keycloak, this url will look something like this: `https://auth.svcs.stage.vydon.dev/admin/realms/vydon-stage`. The pattern is: `<baseurl>/admin/realms/<realm>`
