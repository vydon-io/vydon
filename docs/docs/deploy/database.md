---
title: Vydon Database Setup
description: Learn how to configure a Postgres database for use with Vydon
id: database
hide_title: false
...
This section provides some color to the Postgres environment that is required to run the Vydon database migrations.
...
Vydon API comes equipped with a set of SQL schema migration files that are used to assert the Vydon database on boot up.
...
As a part of this, one may run Vydon API in the `DB_AUTO_MIGRATE=true` mode, where, on startup, Vydon API will run the database migration scripts prior to binding to its port. The same credentials used to do this will be what are used to generally connect to the Vydon API during standard crud operations.
...
Vydon uses [golang-migrate](https://github.com/golang-migrate/migrate) to handle running database migrations. We wrap it inside of our own CLI so that we can easily run it in containerized environments.
...
This table can be changed by providing the `DB_MIGRATIONS_TABLE` environment variable. For example: `DB_MIGRATIONS_TABLE=vydon_api_schema_migrations`.
...
Vydon creates the `schema_migrations` table in the `public` schema and also creates a `vydon_api` custom schema that it dumps all of its table into.
As of the writing of this article, the `vydon_api` schema is hardcoded into the codebase and cannot easily be changed. If this is important to you that it be something else, please reach out to us on Discord.
...
```sql
GRANT CREATE ON SCHEMA public TO vydon_owner;
GRANT USAGE ON SCHEMA public TO vydon_owner;
GRANT CREATE ON DATABASE postgres TO vydon_owner;
```

We can run the migrations with an admin user that inherits from the `vydon_owner` role.

```sql
CREATE ROLE vydon_admin WITH
    LOGIN
    INHERIT
    PASSWORD 'password'
    NOSUPERUSER
    ...
    CONNECTION LIMIT -1
    VALID UNTIL 'infinity';
GRANT vydon_owner TO vydon_user;
```

In order to retain the ownership of the tables to the `vydon_owner`, you'll need to ensure the `DB_MIGRATIONS_OPTIONS=-c role=vydon_owner` is set.
