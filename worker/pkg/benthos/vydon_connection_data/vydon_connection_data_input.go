...
var vydonConnectionDataConfigSpec = service.NewConfigSpec().
	Summary("Streams Vydon connection data").
...
func newVydonConnectionDataInput(
	...
	registry := vydontypes.NewTypeRegistry(logger)

	return service.AutoRetryNacks(&vydonInput{
...
func RegisterVydonConnectionDataInput(
	env *service.Environment,
	vydonConnectApi mgmtv1alpha1connect.ConnectionDataServiceClient,
	logger *slog.Logger,
) error {
	return env.RegisterInput(
		"vydon_connection_data", vydonConnectionDataConfigSpec,
		func(conf *service.ParsedConfig, mgr *service.Resources) (service.Input, error) {
			return newVydonConnectionDataInput(conf, vydonConnectApi, logger)
		},
	)
}
...
func (g *vydonInput) Connect(ctx context.Context) error {
	...
	resp, err := g.vydonConnectApi.GetConnectionDataStream(
		ctx,
...
func (g *vydonInput) Read(ctx context.Context) (*service.Message, service.AckFunc, error) {
	g.recvMut.Lock()
	defer g.recvMut.Unlock()

	if g.vydonConnectApi == nil && g.resp == nil {
	...
	if g.connectionType == string(benthosbuilder_shared.ConnectionTypeDynamodb) {
		...
		msg.MetaSetMut(vydon_metadata.MetaTypeMapStr, keyTypeMap)
...
func (g *vydonInput) Close(ctx context.Context) error {
	// close client
	...
	g.vydonConnectApi = nil // idk if this really matters
	return nil
}

func unmarshalDynamoDBItem(
	item map[string]any,
) (standardMap map[string]any, keyTypeMap map[string]vydon_types.KeyType) {
