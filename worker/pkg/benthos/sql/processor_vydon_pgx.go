...
func vydonToPgxProcessorConfig() *service.ConfigSpec {
...
func RegisterVydonToPgxProcessor(env *service.Environment) error {
	return env.RegisterBatchProcessor(
		"vydon_to_pgx",
		vydonToPgxProcessorConfig(),
		func(conf *service.ParsedConfig, mgr *service.Resources) (service.BatchProcessor, error) {
			proc, err := newVydonToPgxProcessor(conf, mgr)
			if err != nil {
				return nil, err
			}
			return proc, nil
...
type vydonToPgxProcessor struct {
...
func newVydonToPgxProcessor(
	conf *service.ParsedConfig,
	mgr *service.Resources,
) (*vydonToPgxProcessor, error) {
...
func (p *vydonToPgxProcessor) ProcessBatch(
	ctx context.Context,
	...
	for _, msg := range batch {
		...
		newRoot, err := transformVydonToPgx(
			root,
			p.columns,
			p.columnDataTypes,
...
func (m *vydonToPgxProcessor) Close(context.Context) error {
	return nil
}
func transformVydonToPgx(
	root any,
	columns []string,
	columnDataTypes map[string]string,
	columnDefaultProperties map[string]*vydon_benthos.ColumnDefaultProperties,
...
func getPgxValue(
	...
	value, isVydonValue, err := getPgxVydonValue(value)
	if err != nil {
		return nil, err
	}
	if isVydonValue {
...
func getPgxVydonValue(root any) (value any, isVydonValue bool, err error) {
	if valuer, ok := root.(vydontypes.VydonPgxValuer); ok {
		value, err := valuer.ValuePgx()
		if err != nil {
			return nil, false, fmt.Errorf("unable to get PGX value from VydonPgxValuer: %w", err)
		}
...
func getColumnDefaultProperties(
	...
	for key, properties := range columnDefaultPropertiesConfig {
		...
		var colDefaults vydon_benthos.ColumnDefaultProperties
