...
func Test_transformVydonToPgx(t *testing.T) {
	...
	t.Run("transforms values correctly", func(t *testing.T) {
		...
		got, err := transformVydonToPgx(input, columns, columnDataTypes, columnDefaultProperties)
		require.NoError(t, err)
	...
	t.Run("invalid input type", func(t *testing.T) {
		_, err := transformVydonToPgx("not a map", columns, columnDataTypes, columnDefaultProperties)
		require.Error(t, err)
	})
}
...
func Test_VydonToPgxProcessor(t *testing.T) {
	conf := `
columns:
  - id
  - name
...
`
	spec := vydonToPgxProcessorConfig()
	env := service.NewEnvironment()

	procConfig, err := spec.ParseYAML(conf, env)
	require.NoError(t, err)

	proc, err := newVydonToPgxProcessor(procConfig, service.MockResources())
	require.NoError(t, err)

	interval, err := vydontypes.NewInterval()
	require.NoError(t, err)
...
func Test_VydonToPgxProcessor_SubsetColumns(t *testing.T) {
	conf := `
columns:
  - id
  - name
