...
func Test_transformVydonToMysql(t *testing.T) {
	...
	t.Run("handles basic values", func(t *testing.T) {
		...
		result, err := transformVydonToMysql(input, columns, columnDataTypes, columnDefaultProperties)
	...
	t.Run("returns error for invalid root type", func(t *testing.T) {
		result, err := transformVydonToMysql("invalid", columns, columnDataTypes, columnDefaultProperties)
		require.Error(t, err)
...
func Test_getMysqlValue(t *testing.T) {
	t.Run("returns default for column with default transformer", func(t *testing.T) {
		colDefaults := &vydon_benthos.ColumnDefaultProperties{HasDefaultTransformer: true}
...
func Test_VydonToMysqlProcessor(t *testing.T) {
	conf := `
columns:
  - id
  - name
...
`
	spec := vydonToMysqlProcessorConfig()
	env := service.NewEnvironment()

	procConfig, err := spec.ParseYAML(conf, env)
	require.NoError(t, err)

	proc, err := newVydonToMysqlProcessor(procConfig, service.MockResources())
	require.NoError(t, err)
...
func Test_VydonToMysqlProcessor_SubsetColumns(t *testing.T) {
	conf := `
columns:
  - id
  - name
