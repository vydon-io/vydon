...
func Test_transformVydonToMssql(t *testing.T) {
	...
	t.Run("handles basic values", func(t *testing.T) {
		...
		result, err := transformVydonToMssql(input, columns, columnDefaultProperties)
	...
	t.Run("returns error for invalid root type", func(t *testing.T) {
		result, err := transformVydonToMssql("invalid", columns, columnDefaultProperties)
		require.Error(t, err)
...
func Test_VydonToMssqlProcessor(t *testing.T) {
	conf := `
columns:
  - id
  - name
...
`
	spec := vydonToMssqlProcessorConfig()
	env := service.NewEnvironment()

	procConfig, err := spec.ParseYAML(conf, env)
	require.NoError(t, err)

	proc, err := newVydonToMssqlProcessor(procConfig, service.MockResources())
	require.NoError(t, err)
...
func Test_VydonToMssqlProcessor_SubsetColumns(t *testing.T) {
	conf := `
columns:
  - id
  - name
