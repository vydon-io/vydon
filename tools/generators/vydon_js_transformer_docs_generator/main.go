...
const newValue = vydon.{{.BenthosSpec.Name}}(value, {});
{{- else }}
const newValue = vydon.{{.BenthosSpec.Name}}(value, {
...
{{if eq (len .BenthosSpec.Params) 0}}
const newValue = vydon.{{.BenthosSpec.Name}}({});
{{- else }}
const newValue = vydon.{{.BenthosSpec.Name}}({
	{{- range $i, $param := .BenthosSpec.Params -}}
	{{ if $param.HasDefault }}
...
func generateExample(bs *transformers.BenthosSpec) (string, error) {
	...
	t := template.Must(template.New("vydonTransformerExample").Parse(exampleTemplate))
...
hide_title: false
id: javascript
description: Learn about Vydon's javascript transformer
---
<!-- prettier-ignore-start -->
<!--
	Code generated by Vydon vydon_js_transformer_docs_generator.go. DO NOT EDIT.
-->

# Vydon Javascript Transformer Functions

Learn about Vydon's Javascript transformer and generator functions, which provide a wide range of capabilities for data transformation and
...
Vydon's transformer functions allow you to manipulate and transform data values with ease.
...
Vydon's generator functions enable the creation of various data values, facilitating the generation of realistic and diverse data for
...
func generateCode(benthosSpecs []*transformers.BenthosSpec) (string, error) {
	...
	t := template.Must(template.New("vydonTransformerDocs").Parse(docTemplate))
	var out bytes.Buffer
	err := t.Execute(&out, data)
	if err != nil {
		return "", err
