-- name: GetUser :one
SELECT * FROM vydon_api.users
WHERE id = $1;

-- name: GetUserAssociationByProviderSub :one
SELECT * from vydon_api.user_identity_provider_associations
WHERE provider_sub = $1;

-- name: GetUserByProviderSub :one
SELECT u.* from vydon_api.users u
...
SELECT aipa.* FROM vydon_api.user_identity_provider_associations aipa
...
-- name: GetAccount :one
SELECT * from vydon_api.accounts
WHERE id = $1;

-- name: GetPersonalAccountByUserId :one
SELECT a.* from vydon_api.accounts a
...
SELECT aua.* from vydon_api.account_user_associations aua
INNER JOIN vydon_api.accounts a ON a.id = aua.account_id
INNER JOIN vydon_api.users u ON u.id = aua.user_id
...
SELECT * FROM vydon_api.account_invites
